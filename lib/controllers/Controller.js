...
Controller.prototype.redirectTo = function (target, params) {
	var link = this.linkTo(target, params);
	if (!link) {
...
Controller.prototype.redirectToUri = function (uri) {
	var self = this;
	this.saveSession(function () {
		self.$response.header('location', uri);
		self.$response.end(302);
	});
};

Controller.prototype.linkTo = function (target_str, params, absolute) {
	var target = target_str.split(':');
	target_str = [
	...
	var link = this.$router.getTargetURL(target_str, params, this.$request);
	if (absolute && /^\//.test(link)) {
		var hostname = this.$request.getHostLevels().join('.');
		var port = this.$request.getPort();
		link = 'http://' + hostname + (port === 80 ? '' : ':' + port) + link;
	}
	return link;
};

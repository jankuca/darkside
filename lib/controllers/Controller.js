...
var Controller = function (name, router, request, response, service_container) {
	this.router = router;
...
Controller.prototype.callAction = function (action /*, ..args */) {
	...
	var session_repository = this.getService('sessions');
	var session_id = this.request.getCookie('SID') || session_repository.generateId(this.request.getHeaders());
	session_repository.getById(session_id, function (err, session) {
		if (err) {
			log(err.toString());
...
Controller.prototype.getService = function (key) {
	return this.service_container_.getService(key);
};

...
ViewController.prototype.defineViewHelpers = function () {
	...
	view['link_to'] = function (target, params, abs) {
		return controller.linkTo(target, params, abs);
	};
};
...
ViewController.prototype.render = function (status) {
	...
	this.view_stack_.execute(this.view, function (err, rendering) {
		if (err) {
			...
			darkside.Controller.prototype.render.call(this, status, rendering);
		}
	}, this);
};

ViewController.prototype.terminate = function (status, err) {
	if (err) {
		this.setLayout(null);
		this.setTemplatePath('error.eco');
		this.view['status'] = status;
		this.view['message'] = err.message || err;
		this.view['stack'] = err.stack ? err.stack.split('\n').slice(1) : [];
		this.render();

	} else {
		var self = this;
		this.saveSession(function () {
			self.$response.end(status);
		});
	}
};
